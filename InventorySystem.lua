...
-- Location: ServerStorage/Modules/InventorySystem.lua
-- Version: 1.0.0
...
function InventorySystem:EquipItem(inventoryId)
	...
	if item.classRestriction then
		-- Note: Actual class check will be done in InventoryService
		-- Since InventorySystem has no knowledge of the player's class
	end
...
function InventorySystem:CalculateEquipmentStats()
	local stats = {
		...
		maxMp = 0,
		attack = 0,
		defense = 0, -- Can be increased by equipment
		magic = 0
	}

	print("[InventorySystem] Calculating equipment stats")
	...
	for slotType, inventoryId in pairs(self.equippedItems) do
		...
		if item and item.stats then
			...
			print("[InventorySystem] Equipment #" .. itemCount .. ": " .. item.name)
			...
			elseif slotType == EQUIPMENT_SLOTS.ARMOR and item.stats.maxHp then
				stats.armorHp = item.stats.maxHp
			end
		end
	end

	-- Log total stats
	print("[InventorySystem] Total equipment stats:")
