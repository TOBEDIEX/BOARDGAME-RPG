...
-- Version: 8.3.0 (Simplified state management after server-side changes)
...
local state = {
	lastTileId = nil,
	lastTileType = nil,
	isProcessingEvent = false -- Still useful to prevent overlap
}
...
local boardRemotes, tileTriggerEvent -- Only need TileTriggerEvent now
...
local function initRemotes()
	...
	tileTriggerEvent = boardRemotes:WaitForChild("TileTriggerEvent") -- Main event we listen to now
end
...
-- Play card reveal animation (No changes needed)
...
-- Show money animation with existing UI (No changes needed)
local function showMoneyAnimation(amount)
	...
	mainFrame.Visible = true -- Make sure frame is visible
...
-- Function to handle the TileTriggerEvent
local function handleTileTrigger(playerId, tileId, tileType)
	if playerId ~= player.UserId then return end -- Only handle events for the local player
	if state.isProcessingEvent then return end -- Prevent overlap

	state.lastTileId = tileId
	state.lastTileType = tileType
	state.isProcessingEvent = true

	print("[EventTileHandler] Received TileTriggerEvent: TileID=" .. tileId .. ", Type=" .. tileType)

	-- Process event after a short delay
	task.delay(DELAY_BEFORE_UI, function()
		if tileType == "money" then
			local moneyAmount = getRandomMoneyAmount()
			createNotification("You landed on a money tile! Received " .. moneyAmount .. " coins", NOTIFY_DURATION, MONEY_COLOR)
			showMoneyAnimation(moneyAmount)
		elseif tileType == "item" then
			createNotification("You landed on an item tile! Getting random item...", NOTIFY_DURATION, ITEM_COLOR)
			-- Request item from server
			itemEventRemote:FireServer("giveRandomItem")
		elseif tileType == "shop" then
			createNotification("You landed on a shop tile", NOTIFY_DURATION, SHOP_COLOR)
			-- TODO: Open shop UI
		elseif tileType == "battle" then
			createNotification("You landed on a battle tile! Prepare to fight", NOTIFY_DURATION, BATTLE_COLOR)
			-- TODO: Initiate battle sequence
		elseif tileType == "casino" then
			createNotification("You landed on a casino tile! Try your luck?", NOTIFY_DURATION, CASINO_COLOR)
			-- TODO: Open casino UI
		else
			-- Handle other tile types or normal tiles if needed
			print("[EventTileHandler] Landed on tile type: " .. tileType)
		end
...
local function setupConnections()
	-- Money event (still needed if server sends money directly)
	...
	-- *** Listen for TileTriggerEvent ***
	tileTriggerEvent.OnClientEvent:Connect(handleTileTrigger)
...
-- Expose function for compatibility (though it might not be needed anymore)
_G.HandleItemTile = function()
	print("[EventTileHandler] _G.HandleItemTile called (likely deprecated)")
end

print("[EventTileHandler] Client Event Tile Handler Initialized (v8.3.0)")
