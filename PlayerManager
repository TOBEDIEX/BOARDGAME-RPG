...
-- Version: 1.2.0 (Improved Death & Respawn Handling)
...
function PlayerManager:InitializeRemotes()
	...
	self.playerRespawnedEvent = ensureRemoteEvent(uiRemotes, "PlayerRespawned") -- New for respawn notification
...
function PlayerManager:GetDefaultStatsForClass(className)
	...
	if classStats then
		...
		print(string.format("[PlayerManager DEBUG] GetDefaultStatsForClass: Created stats for %s using ClassData: MaxHP=%d, HP=%d, MaxMP=%d, MP=%d, ATK=%d, DEF=%d, MAGIC=%d",
			className, stats.maxHp, stats.hp, stats.maxMp, stats.mp, stats.attack,
			stats.defense, stats.magic))
		return stats
	else
...
-- Get all player data (for GameManager)
...
function PlayerManager:SetPlayerClass(player, className)
	...
	if classSystem then
		...
		-- Save money before reset
		local currentMoney = playerData.stats.money
...
function PlayerManager:ApplyEquipmentStatsToPlayer(player)
	...
	-- IMPORTANT: Save current money before stats calculation
	local currentMoney = playerData.stats.money or 100
	print("[PlayerManager DEBUG] Saving current money value before stat calculation:", currentMoney)
	...
	-- IMPORTANT: Restore saved money after stats calculation
	playerData.stats.money = currentMoney
	print("[PlayerManager DEBUG] Restored money value after stat calculation:", playerData.stats.money)
...
function PlayerManager:UpdatePlayerStats(player, statUpdates)
	...
	if hpChanged and playerData.stats.hp <= 0 and playerData.isAlive then print("[PlayerManager DEBUG] Player died due to UpdatePlayerStats."); self:HandlePlayerDeath(player) end
	return true
end
...
function PlayerManager:SyncPlayerStats(player)
	...
	local statsToSync = {
		...
		exp = playerData.stats.exp, nextLevelExp = playerData.stats.nextLevelExp -- Include EXP data for UI updates
	}
...
function PlayerManager:UpdatePlayerHP(player, newHP)
	...
	if playerData.stats.hp <= 0 and playerData.isAlive then print("[PlayerManager DEBUG] Player died due to UpdatePlayerHP."); self:HandlePlayerDeath(player) end
	return true
end
...
-- Handle Player Death (Improved Respawn logic)
...
function PlayerManager:HandlePlayerDeath(player)
	...
	-- If the player is already in dead state, don't process again
	if not playerData.isAlive then
		print("[PlayerManager DEBUG] Player", player.Name, "is already in dead state.")
		return false
	...
	-- Store additional death state information for respawn
	playerData.deathState = {
		timeOfDeath = os.time(),
		deathPosition = player.Character and player.Character:FindFirstChild("HumanoidRootPart") and
			player.Character:FindFirstChild("HumanoidRootPart").Position or nil
	}

	-- Notify other systems about player death
	local gameManager = self.gameManager or _G.GameManager
	if gameManager then
		if gameManager.turnSystem then
			-- Notify turn system about player death (skip turn, not remove)
			gameManager.turnSystem:OnPlayerDeath(player.UserId)
		end
	end

	-- Schedule respawn after delay
	task.delay(5, function()
		if not self.players[player.UserId] or not player:IsDescendantOf(Players) then
			print("[PlayerManager DEBUG] Player", player.Name, "left the game before respawn.")
			return
		end -- *** FIXED: Changed } to end ***

		print("[PlayerManager DEBUG] Respawning player:", player.Name)

		-- Store current turn data before reset
		local turnData = nil
		if gameManager and gameManager.turnSystem then
			turnData = {
				isPlayerTurn = gameManager.turnSystem:IsPlayerTurn(player.UserId),
				currentTurnIndex = gameManager.turnSystem.currentTurnIndex
			}
		end -- *** FIXED: Changed } to end ***
		...
		-- Force character reload to clear any stuck events
		player:LoadCharacter()

		-- Wait for character to load
		task.wait(0.5)

		-- Find checkpoint system for respawn
		local checkpointSystem = gameManager and gameManager.checkpointSystem or _G.CheckpointSystem
		local respawnTileId = 1 -- Default to start tile

		-- Use checkpoint system if available
		if checkpointSystem then
			respawnTileId = checkpointSystem:GetPlayerRespawnTileId(player)
			local respawnPosition = checkpointSystem:GetPlayerRespawnPosition(player)

			-- Move player to respawn position
			local character = player.Character
			if character and respawnPosition then
				local humanoidRootPart = character:FindFirstChild("HumanoidRootPart")
				if humanoidRootPart then
					humanoidRootPart.CFrame = CFrame.new(respawnPosition + Vector3.new(0, 3, 0))
					print("[PlayerManager DEBUG] Teleported player", player.Name, "to respawn position at tile", respawnTileId)
				end -- *** FIXED: Changed } to end ***
			end -- *** FIXED: Changed } to end ***

			-- Update board system with player's position
			local boardSystem = gameManager and gameManager.boardSystem or _G.BoardSystem
			if boardSystem then
				boardSystem:SetPlayerPosition(player.UserId, respawnTileId, nil)
				print("[PlayerManager DEBUG] Updated player", player.Name, "position in BoardSystem to tile", respawnTileId)

				-- Notify all clients to update player position in UI
				local remotes = ReplicatedStorage:WaitForChild("Remotes")
				local boardRemotes = remotes:WaitForChild("BoardRemotes")
				if boardRemotes:FindFirstChild("UpdatePlayerPosition") then
					boardRemotes.UpdatePlayerPosition:FireAllClients(player.UserId, respawnTileId)
				end -- *** FIXED: Changed } to end ***
			end -- *** FIXED: Changed } to end ***
		else -- *** FIXED: Removed { ***
			print("[PlayerManager DEBUG] CheckpointSystem not found, using default respawn logic.")
		end -- *** FIXED: Changed } to end, completing the if/else structure ***

		-- Reconnect health system
		self:ConnectPlayerHealth(player)

		-- Update player stats to client
		self:SyncPlayerStats(player)

		-- Reset camera system if necessary
		local cameraScript = player:FindFirstChild("PlayerScripts") and
			player.PlayerScripts:FindFirstChild("CameraSystem")
		if cameraScript and cameraScript.Disabled then
			cameraScript.Disabled = false -- Re-enable camera system
		end -- *** FIXED: Changed } to end ***

		-- Notify client about respawn with necessary data
		if self.playerRespawnedEvent then
			self.playerRespawnedEvent:FireClient(player, {
				respawnTileId = respawnTileId,
				stats = {
		...
		-- Notify turn system about player respawn
		if gameManager and gameManager.turnSystem then
			gameManager.turnSystem:OnPlayerRespawn(player.UserId)
		end -- *** FIXED: Changed } to end ***

		print("[PlayerManager DEBUG] Player", player.Name, "successfully respawned.")
	end) -- Closes the task.delay function

	return true
end -- *** FIXED: Changed } to end, closing the HandlePlayerDeath function ***
