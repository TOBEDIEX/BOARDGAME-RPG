...
-- ตัวจัดการเหตุการณ์ช่องพิเศษฝั่งเซิร์ฟเวอร์ (สนับสนุนการทำงานแบบไคลเอนต์เป็นหลัก)
...
local function getPlayerManager()
	...
	-- ถ้าไม่พบ สร้าง stub
	log("ไม่พบ PlayerManager สร้าง stub")
	return {
		GetPlayerData = function(_, player) return {stats = {money = 1000}} end,
		UpdatePlayerStats = function(_, player, stats)
			log("⚠️ [STUB] UpdatePlayerStats สำหรับ " .. player.Name .. " - เงินใหม่: " .. (stats.money or "N/A"))
			return true
		end
	}
end
...
local function updatePlayerMoney(player, amount)
	...
	-- ถ้าวิธี 1 ไม่สำเร็จ ลองวิธีอื่น
	if not success then
		-- วิธี 2: อัพเดทโดยตรง
...
local function testUpdateMoney(player, amount)
	...
	-- สุ่มจำนวนเงินถ้าไม่ได้ระบุ
	amount = amount or math.random(500, 2000)

	log("ทดสอบอัพเดทเงิน " .. player.Name .. " จำนวน " .. amount)

	-- อัพเดทเงินและส่ง event กลับไปยัง client
...
log("ระบบช่องพิเศษฝั่งเซิร์ฟเวอร์เริ่มต้นแล้ว - พิมพ์ _G.TestUpdateMoney() ในแถบคำสั่งเพื่อทดสอบ")v
